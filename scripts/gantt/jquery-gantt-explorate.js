  ...
  var lazyload = {
    ...
    renderLazyLoad: function renderLazyLoad(data, startDate, endDate) {
      ...
      for (var i = 0; i <= num; i += 1) {
        var templateHeaderMonth = $$1('<div>', {
          class: 'gantt-header-month'
        });
        var templateHeaderDay = $$1('<div>', {
          class: 'gantt-header-day'
        });
        var templateHeaderDayMin = $$1('<div>', {
          class: 'gantt-header-day-min'
        });
        ...
        var monthTemplate = $$1(templateHeaderMonth).text(monthName + ' ' + year).css({
          width: monthWidth
        });
      ...
      $$1(templateHeaderMonths, el).css({
        width: totalWidth
      });
      $$1(templateHeaderDays, el).css({
        width: totalWidth
      });
      $$1(templateHeaderDaysMin, el).css({
        width: totalWidth
      });

      var templateGrid = $$1('.gantt-grid', el);
      $$1(templateGrid, el).css({
        width: totalWidth
      });

      var templateGridRow = $$1('.gantt-grid-row', el);
      $$1(templateGridRow, el).css({
        width: this.gridDefaults.gridtotalWidth
      });

      var templateGridCol = $$1('.gantt-grid-col', el);
      $$1(templateGridCol, el).css({
        height: this.gridDefaults.gridtotalHeight
      });

      $$1('.gantt-event-row', el).css({
        width: this.gridDefaults.gridtotalWidth
      });
    ...
    renderLazyLoadContainer: function renderLazyLoadContainer() {
      ...
      var templateWrapper = $$1('<div>', {
        class: 'gantt-wrapper'
      });

      var templateHeader = $$1('<div>', {
        class: 'gantt-header'
      });
      var templateHeaderMonths = $$1('<div>', {
        class: 'gantt-header-months'
      });
      var templateHeaderDays = $$1('<div>', {
        class: 'gantt-header-days'
      });
      var templateHeaderDaysMin = $$1('<div>', {
        class: 'gantt-header-days-min'
      });
      ...
      var templateGrid = $$1('<div>', {
        class: 'gantt-grid'
      });
      var templateEvents = $$1('<div>', {
        class: 'gantt-events'
      });

      var templateLoading = $$1('<div>', {
        class: 'gantt-loading',
        style: 'display: none'
      });
      ...
      var templateGridCols = $$1('<div>', {
        class: 'gantt-grid-cols'
      });
      var templateGridRows = $$1('<div>', {
        class: 'gantt-grid-rows'
      });
    ...
    renderLazyLoadEvents: function renderLazyLoadEvents(data) {
      ...
      data.forEach(function (element) {
        var itemStartDate = new Date(element.origin_departs);

        var templateEventRow = $$1('<div>', {
          class: 'gantt-event-row',
          width: this.totalWidth
        });
        var templateEvent = $$1('<div>', {
          class: 'gantt-event'
        });

        var transit_timeWidth = (parseInt(element.transit_time, 10) + 1) * this.config.cellWidth;
        var remDay = this.dateDiffInDays(this.config.startDate, itemStartDate);

        var tooltipData = $$1.extend(element.tooltipData, {
          price: element.price
        });
        var ev_label = element.transit_time + ' Days Transit Time';

        var eventBlock = $$1('<a>', {
            class: this.format('gantt-event-block {0}', carrierType),
            width: transit_timeWidth + 'px',
            href: '/' + element.url,
            target: '_blank'
          }).text(ev_label)
          .css('line-height', this.config.cellHeight - 28 + 'px')
          .data('tooltip', this.tooltipView(tooltipData));

        var eventIcon = $$1('<div class="gantt-event-icon"><div class="' + carrierType + '"></div></div>');
    ...
    createClass(Gantt, [{
      ...
      value: function renderHeader(num) {
        ...
        for (var i = 0; i <= num; i += 1) {
          ...
          var dayMinTemplate = $$1(templateHeaderDayMin).text(weekOfdayName).css({
            width: this.config.cellWidth
          });
      ...
      value: function renderEvents(num) {
        ...
        data.forEach(function (el) {
          var itemStartDate = new Date(el.origin_departs);
          ...
          var transit_timeWidth = (parseInt(el.transit_time, 10) + 1) * this.config.cellWidth;
          var remDay = this.dateDiffInDays(this.config.startDate, itemStartDate);
          ...
          var carrierType = el.carrier_scac.toLowerCase();

          /*switch( el.carrier_scac ){
          	case "OOLU" : 
          		carrierType = 'oolu';
          		break;
          	case "NYKS" : 
          	...
          	case "QAED" : 
          		carrierType = 'qaud';
          		break;
          	default: carrierType = 'default'
          }*/

          var ev_label = el.transit_time + ' Days Transit';

          //var book_url = '/explorate_application/index.cfm/booking/create/sid/' + el.schedule_id;

          var eventBlock = $$1('<a>', {
            class: 'gantt-event-block ' + carrierType,
            width: transit_timeWidth + 'px'
            /*,
                      href: 'sid/' + el.schedule_id,
                      target: '_blank'*/
          }).html('<span class="book-ev" data-sid="' + el.schedule_id + '">' + ev_label + '</span>').css('line-height', this.config.cellHeight - 28 + 'px').data('tooltip', this.tooltipView(tooltipData))
      ...
      value: function tooltipView(data) {
        var template = '' + '<div class="gantt-tooltip">' + '    <div class="tooltip-content">' + '        <img src="{0}" alt="tooltip-img">' + '        <span class="title">{1}</span>' + '        <div class="desc">' + '            {2} <br> {3} <br> {4}' + '        </div>' + '    </div>' + '    <div class="tooltip-action">' + '        <span>Dep: <span class="desc">{5}</span></span><br>' + '        <span>Arr: <span class="desc">{6}</span></span>' + '        <div class="price">' + '            <div class="tl">{7}</div>' + '            <div class="eur">{8}</div>' + '        </div>' + '    </div>' + '</div>';

        var html = this.format(template, data.image, data.vessel_name, data.desc[0], data.desc[1], data.desc[2], data.dates.begin, data.dates.end, this.format('{0} {1}', data.price.converted.price, data.price.converted.priceType), this.format('{0} {1}', data.price.original.price, data.price.original.priceType));
        return html;
      }
    }, {
      key: 'tooltipHover',
